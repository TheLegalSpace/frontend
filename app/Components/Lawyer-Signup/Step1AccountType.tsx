          ...
          <div className="relative">
            ...
            >
              <span className={selected ? "text-gray-900" : "text-gray-400"}>
                {selected === "lawyer" ? "Independent Lawyer" : selected === "firm" ? "Law Firm" : "Select account type"}
              </span>
            ...
            {open && (
              <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-100 rounded-xl shadow-lg z-10 overflow-hidden">
                <button
                  onClick={() => { setSelected("firm"); setOpen(false); setError(""); }}
                  className="w-full text-left px-4 py-3 text-[13px] text-gray-700 hover:bg-gray-50 transition-colors"
                >
                  Law Firm
                </button>
                <button
                  onClick={() => { setSelected("lawyer"); setOpen(false); setError(""); }}
